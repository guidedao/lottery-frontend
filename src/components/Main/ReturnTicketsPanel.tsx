...
export default function ReturnTicketsPanel() {
    ...
    return (
        <div className='flex flex-col gap-3 p-6 bg-white rounded-lg shadow-md mt-4'>
            <h3 className='text-lg font-semibold text-gray-900'>Return Tickets</h3>
            <div className='flex items-end gap-2'>
                <div className='flex-1'>
                    ...
                    <Input
                        ...
                        className='w-full'
                    />
                    <div className='text-xs text-gray-500 mt-1'>Max: {maxReturnable}</div>
                </div>
                <Button
                    onClick={handleReturnSelected}
                    disabled={isLoading || !isReturnAllowed || amount <= 0 || amount > maxReturnable}
                    variant='secondary'>
                    {isLoading ? 'Returning…' : 'Return Selected'}
                </Button>
                <Button onClick={handleReturnAll} disabled={isLoading || !isReturnAllowed} variant='destructive'>
                    {isLoading ? 'Returning…' : `Return All (${maxReturnable})`}
                </Button>
            </div>
            {!isReturnAllowed && <p className='text-xs text-gray-600'>Return period has ended.</p>}
            {isError && error && <p className='text-xs text-red-600'>Error: {error.message}</p>}
            {isSuccess && <p className='text-xs text-green-600'>Tickets returned successfully.</p>}
        </div>
    );
}
